...
{
	do_malloc(slot/thread,((jbl_uint32)(1<<base))/thread,small,large,huge);
	a++;
    return NULL;
}
int mainn()
...
//	pl();
	...
	pthread_t t[thread];
	for(jbl_uint32 i=0;i<thread;pthread_create(&t[i],NULL,do_thread,NULL),++i);
	for(jbl_uint32 i=0;i<thread;pthread_join(t[i],NULL),++i);
#else
