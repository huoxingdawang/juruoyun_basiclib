...
#define                 get_page_i(page,chunk)      ((jbl_uint16)(((jbl_pointer_int)((char*)page-(char*)chunk))>>12))
/*******************************************************************************************/
/*                            全局变量定义                                                */
/*******************************************************************************************/
...
jbl_malloc_size_type jbl_malloc_size(void* ptr)
{
    ...
    if(chunk->map[i]&0X20000000ULL)//small
...
//printf("%d\n",__LINE__);
    ...
    if(3072<size_now&&size_now<=2093056&&3072<size&&size<=2093056)//全在large
    {
        ...
        jbl_uint16 n=chunk->map[i]&(0X1FFULL);                //有的
        jbl_uint16 page=(jbl_uint16)(((size&(0X1FFULL))!=0)+(size>>12));    //现在需要page个page
...
//printf("%d\t0X%llX\n",__LINE__,ptr);
    ...
    else
    {
        ...
        jbl_malloc_chunk_struct *chunk=aligned_to_2M(page);
        jbl_uint16 i=get_page_i(page,chunk);
        if(chunk->map[i]&0X20000000ULL)     __jbl_free_small(ptr);
...
void *__jbl_malloc_page(jbl_uint16 nums,jbl_uint8 type)//type为0XFF用于large，type为其他用于small
{
    ...
    if(type==0XFF)
        chunk0->map[i0]=tmp=(0X40000000U)|(nums&0X1FFU);            //标记[31,30]U[9,0] 10b<<29
    else
        chunk0->map[i0]=(0X20000000U)|(type&0X1FFU),                //标记[31,30]U[9,0] 01b<<29
        tmp=(0X60000000U)|(type);                                     //                  11b<<29
    for(jbl_uint16 i=1;i<nums;++i)
        chunk0->map[i+i0]=tmp|(0X1FFU&((jbl_uint32)i<<10));           //标记[31,30]U[19,10]U[9,0]
    
    return ((char*)chunk0)+(i0<<12);                                      //返回
}
//释放一个page
jbl_uint16 __jbl_free_page(void *ptr)
{
    ...
    jbl_uint16 n=chunk->map[i]&(0X1FFULL);                              //获取连续page数量
    chunk->free_pages=(jbl_uint16)(n+chunk->free_pages);                //标记以释放
    if(chunk->map[i]&0X20000000ULL)n=jbl_malloc_small_bins[n].pages;    //如果是small n指的是type，取到num    
        ...
        for(jbl_malloc_free_slot *ptr=__jbl_malloc_heap.slot[type].next;ptr;)        //遍历每一个small
        {
            ...
            if(cnt==jbl_malloc_small_bins[type].count)                            //如果small都闲置，把father page所属的small全部移出slot，并标记释放所有page
            {
                ...
                for(jbl_malloc_free_slot *ptr2=__jbl_malloc_heap.slot[type].next,*ptr3=NULL;ptr2;)    //遍历每一个small
                {
                    ...
                    jbl_uint16 i2=(jbl_uint16)(((jbl_malloc_size_type)((char*)page2-(char*)chunk2))>>12);        //计算page编号
                    jbl_uint16 fi2=(jbl_uint16)(i2-(jbl_uint32)((chunk2->map[i2]>>10)&0X1FFULL));                    //计算father page编号
